...
export async function escalateStuckTickets(orch: Orchestrator, projectId: string): Promise<void> {
  await orch.guard(`escalate:${projectId}`, projectId, async () => {
    ...
    if (principalQa) {
      // QA has no per-ticket assignee the way engineer does -- there's
      // exactly one QA agent per project -- so this branch targets
      // WorkItem.qaAssigneeAgentId instead of assigneeAgentId, and reads
      // the current QA agent (for the "by X" wording below) via
      // findRoleAgent rather than off the ticket.
      ...
      if (stuckQa.length) {
        ...
        for (const item of stuckQa) {
          ...
          await board.addComment(
            ...
            "system",
            "Custos",
            `Escalated to ${agentStore.displayName(principalQa)} after ${ESCALATION_THRESHOLD}+ consecutive failed QA attempts by ${regularQa ? agentStore.displayName(regularQa) : "the regular QA agent"}. This run uses the "principal" fallback set (real Anthropic usage) -- see the ticket's comment history and prior PR for what's already been tried.`,
          );
          orch.emit("activity", projectId, { text: `Escalated "${item.title}" to ${agentStore.displayName(principalQa)} after ${ESCALATION_THRESHOLD}+ failed QA attempts` });
        }
      }
    }
  });
