  ...
  async groomBacklog(projectId: string): Promise<void> {
    await this.guard(`groom:${projectId}`, projectId, async (signal) => {
      ...
      const prompt = [
        ...
        "",
        "This output is read only by other software, never by a human — do not write any reasoning, analysis, or summary as prose. Do not narrate what you're about to do or explain your thinking. Any decision worth recording belongs in the block's own `comments`/`revise` fields, not around it. Respond with nothing but the required fenced block below; anything else risks running out of length before you reach it, which fails the entire run.",
        "",
  ...
  async assignReady(projectId: string): Promise<void> {
    await this.guard(`assign:${projectId}`, projectId, async (signal) => {
      ...
      const prompt = [
        ...
        "",
        "This output is read only by other software, never by a human — do not write any reasoning, analysis, or summary as prose. Do not narrate what you're about to do or explain your thinking. Respond with nothing but the required fenced block below; anything else risks running out of length before you reach it, which fails the entire run.",
        "",
