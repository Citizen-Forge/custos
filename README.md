...
- **Remote control, organized into projects** — Anthropic's own Remote Control feature refuses to work once `ANTHROPIC_BASE_URL` points anywhere but `api.anthropic.com` (a deliberate restriction as of Claude Code v2.1.196), so it can't be used alongside this proxy. Custos runs its own instead: **projects** are workspace folders (e.g. one per app you work on), and each project can have any number of **chats** running concurrently -- every chat spawns `claude` in its own real pseudo-terminal inside the container, scoped to that project's folder, and streams it over a WebSocket (the same pattern as ttyd/gotty) so you can steer it from your phone, another computer, or the Custos desktop app (`custos-desktop/`, a separate Electron client -- see its own README). Each spawned chat's own `ANTHROPIC_BASE_URL` points back at Custos itself, so it gets the same permission gating/memory/routing as any other session.
...
Set `CUSTOS_WORKSPACE` (in `.env` or your shell) to the host directory you want reachable remotely -- it gets bind-mounted to `/workspace`, which is the root every project's own subfolder lives under:
...
- Projects and chat metadata (titles, which folder, when created) persist in `data/`, but a chat's *live* process doesn't -- restarting the container ends every running chat. They stay listed and can be reopened, just not resumed mid-process (see Remote control above).
