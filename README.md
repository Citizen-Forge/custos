...
Custos's gateway is the runtime substrate every orchestrator-spawned agent sits on: every chat-turn `claude -p` spawn, every PM agent run, every classifier or curator call -- they all point their own `ANTHROPIC_BASE_URL` at this instance, so the orchestrator can route, gate, and observe them through one path.
...
- **Context memory** — every exchange the gateway serves is logged; a periodic curator extracts durable facts via an LLM and embeds them into Qdrant; a `UserPromptSubmit` hook does semantic search on each new prompt and injects relevant memory back into context. Shared across all the agents in a project, so what DevOps learns (where the repo is) reaches the engineers and what QA works out (how to run the suite) survives past its own run.
...
- **Admin UI** at `/admin` — provider configuration (presets for Anthropic and every OpenAI-compatible source above), embed model, Anthropic OAuth/API key surface. Changes take effect immediately, no restart -- the server rebuilds its providers/router from the saved config on every admin change.
- **Remote control, organized into projects** — **projects** are workspace folders (e.g. one per app you work on), and each project can have any number of **chats** running concurrently -- each chat turn spawns a one-shot `claude -p` process inside the container, scoped to that project's folder, and streams its parsed events over a WebSocket so you can steer it from a browser, your phone, or the [desktop client](https://github.com/Citizen-Forge/custos-desktop). Turns are stitched into one conversation with `--resume` (Claude Code has no persistent multi-turn headless mode, so each turn really is a fresh process).
  ...
  **Important:** this spawns a *new* claude session inside the Custos container -- it does not attach to, view, or control a Claude Code session you already have running elsewhere (a terminal, VS Code, wherever). A PTY is host-local, so there's no general mechanism to attach from a server-side process to a session on another machine. If you want remote control to reach the same machine and files you actually develop on, run Custos there.
...
- **Client API key, fails closed** — `/v1/messages`, `/hooks/*`, and `/memory/search` (everything an orchestrator-spawned claude subprocess calls into) require a matching `x-api-key` header. The key is set via `CUSTOS_CLIENT_API_KEY` in `.env` (or `data/config.json`'s `clientApiKey` field); without it, every request on that surface is rejected. Without this, anything that could reach the endpoint could burn your configured providers' budget/compute for free. The wire header is the same one Claude Code already sends for real Anthropic API-key auth, repurposed here -- Custos does its own provider auth server-side and ignores whatever value the client meant to send upstream.
...
Even orchestrator-spawned agents — every PM run, every chat turn — go through the gateway rather than each subprocess talking to `api.anthropic.com` directly. Hooks alone can't do multi-provider routing or model-based classification: hooks are per-event scripts/HTTP calls, and what they ship back is too narrow to inform a routing decision. The gateway is the always-on service the orchestrator points subprocess `ANTHROPIC_BASE_URL` at — classifier + memory + spend telemetry are side effects of sitting in the path.
...
1. **Connect Anthropic** — click "Connect via OAuth" in the admin UI's Anthropic card (authenticates as your own Claude subscription the same way Claude Code's CLI login does: same client_id, same `claude.ai/oauth/authorize` flow -- this gateway manages upstream auth for orchestrated subprocess runs, not for reselling or multiplying that session elsewhere) or paste in an API key as a fallback. If you never connect either, the gateway falls back to importing the OAuth token Claude Code itself is already logged in with from `~/.claude/.credentials.json`.
2. **Add/edit model provider instances** — defaults point at a local Ollama; add OpenAI, DeepSeek, Gemini, Groq, Mistral, xAI, OpenRouter, or a custom endpoint via the preset dropdown, with an API key if that provider needs one. Set each instance's `maxConcurrent`, `rpmLimit`, and `costType` to match its real-world behaviour.
3. **Set `CUSTOS_CLIENT_API_KEY`** in `.env` (or `data/config.json`'s `clientApiKey` field) -- required: `/v1/messages`/`/hooks/*`/`/memory/search` reject every request until a key exists. Anything you generate once becomes the auth header every spawned subprocess gets, so pick up the same value from `ANTHROPIC_API_KEY` in any environment where you want to route traffic through the gateway.
4. On a fresh install, the defaults are sane enough to just spin up the container and click around. The OAuth flow above is the main thing to do before any real work, because no Anthropic-backed agent will start until `anthropic.oauth.connected` is true.
...
In the admin UI's **Projects** panel, click **New project** -- this creates a subfolder under `/workspace` (named from the project name, deduplicated if it already exists) and every chat you start in that project runs `claude` there. Click **New chat**, then open the connect link it gives you (or open it on your phone). The chat inherits authentication from the spawn (the gateway sets `ANTHROPIC_API_KEY` to the `x-api-key` it just validated), so no separate `/login` is needed inside the connected session -- provided `~/.claude` is left to its default empty state, which lets Custos project its own credentials into the subprocess's settings file.

Chats within a project (and across different projects) run concurrently -- there's no limit like the old single-session model had. **Stop** ends a chat's live process (its history stays in the project's chat list); **Reopen** starts a fresh `claude` process for that same chat entry in the same folder. Claude Code's own transcripts are persisted at `./data/claude-home`, so a reopen genuinely resumes the conversation via `--resume` rather than starting fresh.
...
## For spawned claude subprocesses

```bash
export ANTHROPIC_BASE_URL=http://localhost:8787
export ANTHROPIC_API_KEY=$CUSTOS_CLIENT_API_KEY
```

`ANTHROPIC_API_KEY` here is not a real Anthropic key -- it's Custos's own client key, sent as `x-api-key` and checked by Custos itself before anything reaches a provider. The real upstream credentials (OAuth or a real API key) are configured separately, server-side, in the admin UI. Custos injects these env vars into every subprocess the orchestrator spawns, so you don't need to set them yourself unless you're using a separate Claude Code install that you want to route through the same gateway.
...
If you're running Custos alongside a *separate* Claude Code install on the same machine and want both to function, note that the user's local `claude-permission-hook.exe` may already be registered on `PreToolUse` with matcher `.*` -- two PreToolUse hooks then fire on every tool call. How Claude Code combines two hook verdicts (whether a `deny` from either wins, or `ask` overrides `allow`, etc.) hasn't been confirmed against the docs -- check that before relying on both being active together.

`PostToolUse` is optional: it only powers the best-effort ask-outcome log described below, nothing else depends on it.

## Known limitations (v1)

- The OpenAI-compatible translation layer handles one text block and one tool call per turn; it doesn't multiplex true parallel tool calls in a single turn. Only verified live against Ollama -- DeepSeek/Gemini/Groq/Mistral/xAI/OpenRouter all claim OpenAI compatibility but haven't been individually tested through this gateway, and tool-calling fidelity in particular can vary by provider.
